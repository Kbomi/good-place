...
export default function RestaurantUpdate() {
  ...
  const { register, handleSubmit, setValue } = useForm<TRestaurant>({
    defaultValues: {},
  });
  ...
  const onSubmitHandler = (data: TRestaurant) => {
    ...
    fetch(`http://localhost:9999/restaurants/${data.id}`, options)
      ...
      .catch((error) => {
        console.log("restaurant update error:", error);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setInitContent(result.description);

        Object.keys(result).forEach((key) =>
          setValue(key as keyof TRestaurant, result[key])
        );
      } catch (error) {
        console.log("restaurant read:", error);
      }
    };
    ...
    <>
      ...
      <Box sx={{ mt: 3, p: 2 }}>
        ...
        >
          ...
          <Grid item xs={12}>
            <Editor initContent={initContent} ref={editorRef} />
          </Grid>
